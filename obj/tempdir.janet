...
(defn td/temp-root
  ...
  (case (os/which)
    ...
    :bsd "/tmp"
    # see comment above function definition
    :posix (td/posix-temp-root)
    (errorf "unrecognized os: %n" (os/which))))

(defn td/mk-temp-dir
  ``
  Try to create a new subdirectory of a system-specific temporary
  ...
  success, returns a path of the newly created subdirectory.
  ``
  [&opt template tries]
  (default template "//////")
  (default tries 5)
...
(comment
  ...
  (peg/match ~(repeat 6 :h)
             (reverse (td/mk-temp-dir)))
  # =>
  @[]

  (peg/match ~(sequence (thru "hello-")
                        (repeat 3 :h))
             (td/mk-temp-dir "hello-///"))
  # =>
  @[]

  (do
    (def [success? _] (protect (td/mk-temp-dir "")))
    (not success?))
  # =>
  true
