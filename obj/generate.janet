...
(import ./render :prefix "")

(defn g/choose-random-helper
  [name]
  (def sym-name (string "data/" name))
  (def sym (symbol sym-name))
  (def entry (dyn sym))
  #
  (get entry :value))

(defn g/choose-random
  []
  (def name (rnd/choose data/names))
  #
  [name (g/choose-random-helper name)])

(defn g/scan-with-random
  [pattern]
  (def results
    (if (empty? pattern)
      data/names
...
(defn g/gen-files-from-call-str
